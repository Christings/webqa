...
class TrAsyCommands(object):
    ...
    **作者:tulianghui**
    ...
    def execute_with_data(self, args, bufsize=4096, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=None, shell=True, call_func=None, call_args=None):
        """
        执行命令并且返回命令输出数据的生成器
        ...
        while not self._is_finshed and read_set:
            try:
                readablelist, writablelist, exceptionlist = select.select(read_set, [], read_set, 0.0)
            ...
            if self._proc.stdout in readablelist:
                ...
                if line == '':
                    ...
                    #sys.stdout.flush()
                    #print 'stdout:', line, len(line)
                    #break
                else:
                    line = line.strip(self._NEW_LINE)
                    # 删除控制台格式控制
    ...
    def execute_without_data(self, args, bufsize=4096, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=None, shell=True, call_func=None, call_args=None):
        """
