...
class SvnPackage:
    ...
    def _svn_diff_analy(self, diff_rs, printrs = False):
        ...
        for line in diff_rs:
            ...
            if("Index: " in line):
                ...
                print("built SvnDiffCodeBlock:" + indexname)
    ...
    def _do_svn(self, cmd_type, parm, timeout = 120):
        ...
        stdoutlog=""
        if cmd_type == "co":
            stdoutlog=child.communicate(input=None)
            self.errorlog = stdoutlog
            print(stdoutlog)
        run_time = 0
...
if __name__ == '__main__':
    mysvn = SvnPackage("qa_svnreader", "New$oGou4U!")
    info = mysvn.svn_info("/search/odin/daemon/webqo/qo_test/.")
    print("info",info)
    ret = mysvn.svn_co('http://svn.sogou-inc.com/svn/websearch4/web/web_qo/branches/web_qo.20180330.a.hdz', '/search/odin/daemon/webqo/qo_test/.')
    print("ret",ret)
