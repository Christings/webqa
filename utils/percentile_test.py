...
if __name__ == '__main__':
    ...
    fp = codecs.open(input_datafile, 'rb', errors='ignore', encoding='utf-8')
    for line in fp:
        if ("Sogou-Observer" in line):
            try:
                ...
                if (not mat is None):
                    timestamp_key = (
                    time.mktime(datetime.datetime.strptime(mat.group(0), "%Y%m%d %H:%M:%S").timetuple()))
                else:
