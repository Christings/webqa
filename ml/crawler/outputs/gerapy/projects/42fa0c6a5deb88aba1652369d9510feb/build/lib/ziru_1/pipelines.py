...
# def update_errorlog(log,task_id):
#     log = log.replace("'", "\\'")
#     db = pymysql.connect('10.144.120.30', 'root', 'Websearch@qa66', 'sogowebqa')
#     cursor = db.cursor()
#     sql = "UPDATE %s set errorlog=CONCAT(errorlog, '%s') where id=%d;" % ('ml_project', log, task_id)
#     cursor.execute(sql)
...
class JsonWriterPipeline(object):
    ...
    def open_spider(self, spider):
        self.file = open('ziru_1_items.json', 'w')
    ...
    def process_item(self, item, spider):
        ...
        # print('2222:',logger.warning(spider))
        print('item1;',item)
        global count
        count += 1
        if spider.settings.get('COUNT_DATA') and count % 100 == 0:
            s.sendto(u"42fa0c6a5deb88aba1652369d9510feb, %s" % count, ('www.anycrawl.info', 3500))
...
class CsvWriterPipeline(object):
    def open_spider(self, spider):
        self.file = open('ziru_1_items.csv', 'w')
